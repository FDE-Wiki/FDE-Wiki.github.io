...
<abbr class="term" title="前沿部署工程师;把 AI/数据系统部署到客户真实环境并对业务结果负责">FDE</abbr> 选型时面对的约束:
- **合规**:数据能否出境、信创要求;
- **算力**:几张什么卡;
- **预算**:许可费、运维成本;
- **延迟**:实时还是准实时;
...
**Coze(扣子,字节)**
- 低代码 <abbr class="term" title="能自主多步调用工具完成任务的 AI">Agent</abbr>/工作流搭建;
- 适合:快速原型、非技术用户;
- 优势:字节生态、中文友好;
- 劣势:定制化、私有化受限。
...
**反模式三:过度设计**
- 一开始就上最复杂方案;
- 解:从 <abbr class="term" title="最小可行交付;先跑通最小系统">MVD</abbr> 开始,演进。
...
每次选型,<abbr class="term" title="前沿部署工程师;把 AI/数据系统部署到客户真实环境并对业务结果负责">FDE</abbr> 填一张:
- 业务需求与北极星;
- 硬约束(合规/算力/预算/延迟);
- 候选方案(≥2);
- 评估维度与打分;
...
- <abbr class="term" title="检索增强生成">RAG</abbr> 选型树:量级→向量库,关系→Graph<abbr class="term" title="检索增强生成">RAG</abbr>,中文→bge,准确→混合+重排;
- <abbr class="term" title="能自主多步调用工具完成任务的 AI">Agent</abbr> 框架树:生产→LangGraph,原型→CrewAI,生态→官方,企业系统→<abbr class="term" title="Anthropic 提出的 Agent 工具接口标准">MCP</abbr>;
- 向量库:Milvus(大规模/信创)/Qdrant/pgvector/ES;
- 推理:vLLM 通用,TensorRT-LLM 极致;
- 部署:云/私有/边缘/<abbr class="term" title="中国国产化替代">信创</abbr>;
