...
一个完整的企业 AI 系统,<abbr class="term" title="前沿部署工程师;把 AI/数据系统部署到客户真实环境并对业务结果负责">FDE</abbr> 需要打通七层技术栈:
...
模型选型是 <abbr class="term" title="前沿部署工程师;把 AI/数据系统部署到客户真实环境并对业务结果负责">FDE</abbr> 在 Design 阶段的第一个关键决策。2026 年的选型空间分三类:
...
- 优势:<abbr class="term" title="中国国产化替代">信创</abbr>合规、中文强、生态完善;
...
**量化(降低部署成本的关键)**
大模型部署成本高,<abbr class="term" title="前沿部署工程师;把 AI/数据系统部署到客户真实环境并对业务结果负责">FDE</abbr> 常用量化压缩:FP16 → INT8 → INT4。主流量化方案:
...
[第 8 章](/ch08)已详述 <abbr class="term" title="能自主多步调用工具完成任务的 AI">Agent</abbr> 编排,这里补充技术栈要点:
...
> ```python
> # 用 Anthropic <abbr class="term" title="Anthropic 提出的 Agent 工具接口标准">MCP</abbr> SDK 暴露工具
> from mcp.server.fastmcp import Fast<abbr class="term" title="Anthropic 提出的 Agent 工具接口标准">MCP</abbr>
...
<abbr class="term" title="前沿部署工程师;把 AI/数据系统部署到客户真实环境并对业务结果负责">FDE</abbr> 要根据客户约束选择部署形态:
...
- 政务、金融、能源、央企硬要求;
- <abbr class="term" title="前沿部署工程师;把 AI/数据系统部署到客户真实环境并对业务结果负责">FDE</abbr> 要做全栈适配验证。
...
面对这么多技术,<abbr class="term" title="前沿部署工程师;把 AI/数据系统部署到客户真实环境并对业务结果负责">FDE</abbr> 需要一个决策框架,避免"选择困难":
...
- 企业 AI 系统是七层栈:基础设施→数据管道→模型→推理→<abbr class="term" title="检索增强生成">RAG</abbr>→<abbr class="term" title="能自主多步调用工具完成任务的 AI">Agent</abbr>→可观测;
- 模型层:闭源 API/开源权重/国产,按合规+算力+中文选型,量化降本(AWQ);
- 推理层:vLLM 是 2026 事实标准,TGI/TensorRT-LLM 备选;
- <abbr class="term" title="检索增强生成">RAG</abbr> 层:向量库(Milvus/Qdrant/pgvector)+ 嵌入(bge)+ 重排 + 混合检索 + Graph<abbr class="term" title="检索增强生成">RAG</abbr>;
- <abbr class="term" title="能自主多步调用工具完成任务的 AI">Agent</abbr> 层:LangGraph/CrewAI + <abbr class="term" title="Anthropic 提出的 Agent 工具接口标准">MCP</abbr> + 护栏 + <abbr class="term" title="人在回路;关键节点人介入">HITL</abbr>;
- 数据管道占 70% 工作量,Temporal/Prefect/Airflow 编排;
- 可观测:日志/追踪/指标 + LLM 专用(LangSmith/Langfuse)+ eval 前移;
- 部署:云/私有/边缘/<abbr class="term" title="中国国产化替代">信创</abbr>按约束选;docker-compose 一键起栈;
