...
const COMPANIES = ['Palantir','OpenAI','Anthropic','Databricks','IBM','Google','Microsoft','AWS','Meta','NVIDIA','Snowflake','阿里云','腾讯云','华为云','百度','字节跳动','商汤','旷视','智谱','月之暗面'];
...
for (const p of pages) {
  ...
  if (slug) {
    slugMeta[slug] = { part: p.part, kind: p.kind };
    for (const c of COMPANIES) {
      if (body.includes(c)) {
        if (!companyIndex.has(c)) companyIndex.set(c, []);
        companyIndex.get(c).push({ text: p.title || '附录', link: '/' + slug });
      }
    }
  }
}
...
fs.writeFileSync(path.join(DOCS, 'companies.md'), companiesMd);
...
for (let idx = 0; idx < orderedParts.length; idx++) {
  ...
  slugMeta[pslug] = { part: partTitle, kind: 'part' };
  g.link = '/' + pslug;
}
...
sidebar.push({ text: '🏢 公司与案例', link: '/companies' });
...
nav.push({ text: '🏢 公司', link: '/companies' });
if (appendixLink) nav.push({ text: '附录', link: appendixLink });

// ---------- 阅读顺序 + 页面元信息(prev/next、面包屑用)----------
const readOrder = [''];
for (const [, g] of orderedParts) {
  readOrder.push(g.link.slice(1));
  for (const it of g.items) readOrder.push(it.link.slice(1));
}
for (const it of topicItems) readOrder.push(it.link.slice(1));
if (appendixLink) readOrder.push('appendix');
readOrder.push('tags');
readOrder.push('companies');
const pageMeta = { '': { part: null, kind: 'home', title: '首页' }, tags: { part: null, kind: 'tags', title: '按标签浏览' }, companies: { part: null, kind: 'companies', title: '公司与案例' } };
for (const [slug, info] of Object.entries(slugMeta)) pageMeta[slug] = { part: info.part || null, kind: info.kind, title: slugTitle[slug] || slug };
...
fs.writeFileSync(
  ...
  'export const readOrder = ' + JSON.stringify(readOrder) + ';\n\n' +
